...
const createCardContent = (object) => {
  ...
  if(orderInfo.inStock!==0){
    ...
    <img class="productImg" src="${imgUrl}" alt="${name}">
    <h2 class="productTitle">${name}</h2>
    <span class="inStock"><img class= "inStockImg" src="./img/icons/check.svg"><span class="boldNumbers">${orderInfo.inStock}</span> left in stock</span>
    <span class="inStock">Price: <span class="boldNumbers">${price}</span> $</span>
    ...
    <img class="productImg" src="${imgUrl}" alt="${name}">
    <h2 class="productTitle">${name}</h2>
    <span class="inStock"><img class= "inStockImg closeStockImg" src="./img/icons/close.svg">${orderInfo.inStock} left in stock</span>
    <span class="inStock">Price: <span class="boldNumbers">${price} </span>$</span>
    <button class="addToCart disabledButton" id="${id}">Add to cart</button>
    <div class="productInfo stats">
    <img class="likeReview" src='./img/icons/like.svg'>
    <div class="cardBottom">
    <p><span class="boldNumbers">${orderInfo.reviews}%</span> Positive reviews</p>
...
closeFilterModal.forEach((e)=>e.addEventListener("click",()=>{
    filterModal.classList.add("displayNone")
    sortModal.classList.add("displayNone")
    sortButton.classList.remove("checkedSortButton");
}))
